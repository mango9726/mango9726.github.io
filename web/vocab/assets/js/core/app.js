...
function save(key, val) {
  ...
  function gradeAnswer(item, q, noXp) {
    ...
    saveGame();
    if (q >= 3) { showCombo(game.combo, mult, !noXp); if (!noXp) awardXp(xp, "answer"); } // Flashcards ไม่ให้ XP
    else hideCombo(); // ตอบผิด/เฉลย ไม่ให้ XP
  }
...
$("fillSkip").classList.remove("hidden");
  ...
  function findStoryWord(word) {
    const forms = storyWordForms(word);
    const lists = [ITEMS, ALL_ITEMS];
    for (let f = 0; f < forms.length; f++) {
      if (COMMON_TH_DICT[forms[f]]) return COMMON_TH_DICT[forms[f]];
